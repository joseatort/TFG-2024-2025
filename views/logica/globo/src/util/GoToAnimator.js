...
define([
    ...
    function (Location,
        ...
        GoToAnimator.prototype.updateRange = function (currentPosition) {
            ...
            //Añadimos un evento para detectar cuando se produce un Goto
            ...
            if (!this.maxAltitudeReachedTime) {
                ...
                if (Math.abs(this.wwd.navigator.range - nextRange) < 1000) {
                ...
                // Calcular el progreso normalizado (0 a 1) hacia el objetivo
                var totalDistance = this.maxAltitude - this.targetPosition.altitude;
                var remainingDistance = this.wwd.navigator.range - this.targetPosition.altitude;
                var progress = remainingDistance / totalDistance;

                elapsedTime = Date.now() - this.maxAltitudeReachedTime;
                if (this.maxAltitude > this.targetPosition.altitude) {

                    nextRange = this.targetPosition.altitude + ((totalDistance - (totalDistance * 0.05)) * Math.log1p(progress));
                    nextRange = Math.max(nextRange, this.targetPosition.altitude);
                } else {
                    nextRange = this.targetPosition.altitude - ((totalDistance - (totalDistance * 0.05)) * Math.log1p(progress));
                    nextRange = Math.min(nextRange, this.targetPosition.altitude);
                }
                this.wwd.navigator.range = !isNaN(nextRange) ? nextRange : this.targetPosition.altitude;
                // We're done if we get withing 1 meter of the desired range.
                continueAnimation = Math.abs(this.wwd.navigator.range - this.targetPosition.altitude) > 1000;
            }
        ...
        GoToAnimator.prototype.updateLocation = function (currentPosition) {
            ...
            // Calcular la distancia total y la distancia restante
            var totalDistance = Location.greatCircleDistance(this.startPosition, this.targetPosition);
            var remainingDistance = Location.greatCircleDistance(currentPosition, this.targetPosition);
        

            // Si queda una distancia significativa, calcular la nueva posición
            if (remainingDistance > 1 / this.wwd.globe.equatorialRadius) {
                var azimuthToTarget = Location.greatCircleAzimuth(currentPosition, this.targetPosition);
        
                // Calcular la distancia a avanzar en este paso
                var stepDistance = remainingDistance * 0.08; // Ajusta el factor para controlar la velocidad
        
                // Calcular la próxima ubicación usando el azimut y la distancia
                var nextLocation = Location.greatCircleLocation(
                    currentPosition,
                    azimuthToTarget,
                    stepDistance,
                    new Location(0, 0)
                ...
                // Actualizar la latitud y longitud del navegador
                nextLatitude = nextLocation.latitude;
                nextLongitude = nextLocation.longitude;
        
                this.wwd.navigator.lookAtLocation.latitude = nextLatitude;
                this.wwd.navigator.lookAtLocation.longitude = nextLongitude;
            }
        
            // Determinar si se ha alcanzado el destino
            if (remainingDistance < 1 / this.wwd.globe.equatorialRadius) {
